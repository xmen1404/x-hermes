...
public:
  ...
  Config(const std::string &filepath) { node_ = YAML::LoadFile(filepath); }
...
namespace YAML {
template <> struct convert<xhermes::config::Config> {
  static Node encode(const xhermes::config::Config &rhs) { return rhs.node_; }
